...
export type GroupAttendanceReportProps = {
  groupName: string
  area: Area
  /** Por defecto enlace a estadísticas. Use `null` para ocultar (p. ej. vista embebida en manager). */
  backLink?: { href: string; label: string } | null
}

const DEFAULT_BACK = { href: "/estadisticas", label: "Volver" }

export function GroupAttendanceReport({ groupName, area, backLink = DEFAULT_BACK }: GroupAttendanceReportProps) {
  ...
  async function loadGroupData() {
    setLoading(true)
    try {
      console.log("[GroupAttendanceReport] Loading data for group:", groupName, "in area:", area)
      ...
      console.log("[GroupAttendanceReport] Found group:", !!group, "with", group?.participants.length || 0, "participants")
  ...
  function handleDownloadExcel(selectedColumns: string[]) {
    ...
    const data = Array.from(userMap.values()).map(({ record: r, count }) => {
      const row: Record<string, unknown> = { Nombres: r.nombres, "Total Asistencias": count }
      ...
      <div className="container mx-auto p-6">
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <Button onClick={() => setShowExcelDialog(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white gap-2 shrink-0">
